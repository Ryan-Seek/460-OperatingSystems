...
{ 
  if(p == 0)
  {
    printf("Cannot enter child into current process list.\n");
    return -1;//could not enter into list
  }

  if(running->child)//current process has children
  {
  ...
  return 1;//was able to enter process into list.
}
...
int printChild()
{
  PROC *cur = running, *temp;
  temp = cur->child;
  printf("ChildList of proc #%d: ", cur->pid);
...
{
  printf("------- menu --------\n");
  printf("switch fork exit sleep wakeup wait\n");
  printf("---------------------\n");
}  
